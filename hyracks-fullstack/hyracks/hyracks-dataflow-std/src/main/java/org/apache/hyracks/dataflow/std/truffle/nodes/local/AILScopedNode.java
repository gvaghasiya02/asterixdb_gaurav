 ...
 * under the License.
 */
package org.apache.hyracks.dataflow.std.truffle.nodes.local;
...
public abstract class AILScopedNode extends Node {

    /**
     * Index to the the {@link AILBlockNode#getDeclaredLocalVariables() block's variables} that
     ...
     */
    @ExportMessage
    boolean accepts(@Shared("node") @Cached(value = "this", adopt = false) AILScopedNode cachedNode) {
    ...
    final Object getScope(Frame frame, boolean nodeEnter,
            @Shared("node") @Cached(value = "this", adopt = false) AILScopedNode cachedNode,
            @Cached(value = "this.findBlock()", adopt = false, allowUncached = true) Node blockNode) {
        if (blockNode instanceof AILBlockNode) {
            return new VariablesObject(frame, cachedNode, nodeEnter, (AILBlockNode) blockNode);
        } else {
            return new ArgumentsObject(frame, (AILRootNode) blockNode);
        }
    }
    ...
    final boolean hasRootInstance(@SuppressWarnings("unused") Frame frame) {
        ...
        return AILContext.get(this).getFunctionRegistry().getFunction(functionName) != null;
    }
    ...
    final Object getRootInstance(@SuppressWarnings("unused") Frame frame) throws UnsupportedMessageException {
        String functionName = getRootNode().getName();
        // The instance of the current RootNode is a function of the same name.
        Object function = AILContext.get(this).getFunctionRegistry().getFunction(functionName);
        if (function != null) {
            return function;
        } else {
    ...
    /**
     * Find block of this node. Traverse the parent chain and find the first {@link AILBlockNode}. If
    ...
    public final Node findBlock() {
        ...
        while (parent != null) {
            if (parent instanceof AILBlockNode) {
                break;
            }
    ...
    /**
     * Set the index to the the {@link AILBlockNode#getDeclaredLocalVariables() block's variables}
    ...
    /**
     * Scope of function arguments. This scope is provided by nodes just under a {@link AILRootNode},
     * outside of a {@link AILBlockNode block}.
     * <p>
     * The arguments declared by {@link AILRootNode#getDeclaredArguments() root node} are provided as
     * scope members.
     */
    ...
    static final class ArgumentsObject implements TruffleObject {
        ...
        protected final AILRootNode root;

        /**
         * The arguments depend on the current frame and root node.
         */
        ArgumentsObject(Frame frame, AILRootNode root) {
            this.frame = frame;
            this.root = root;
        }
         ...
         */
        @ExportMessage
        boolean accepts(@Cached(value = "this.root", adopt = false) AILRootNode cachedRoot) {
        ...
        Object getMembers(@SuppressWarnings("unused") boolean includeInternal) {
            AILWriteLocalVariableNode[] writeNodes = root.getDeclaredArguments();
        ...
        int findArgumentIndex(String member) {
            ...
            for (int i = 0; i < writeNodes.length; i++) {
                AILWriteLocalVariableNode writeNode = writeNodes[i];
    ...
    static final class VariablesObject implements TruffleObject {
        ...
        protected final AILScopedNode node; // the current node
        final boolean nodeEnter; // whether the node was entered or is about to be exited
        protected final AILBlockNode block; // the inner-most block of the current node

        VariablesObject(Frame frame, AILScopedNode node, boolean nodeEnter, AILBlockNode blockNode) {
            this.frame = frame;
            this.node = node;
         ...
         */
        @ExportMessage
        boolean accepts(@Cached(value = "this.node", adopt = false) AILScopedNode cachedNode,
        ...
        Object toDisplayString(boolean allowSideEffects,
                @Shared("block") @Cached(value = "this.block", adopt = false) AILBlockNode cachedBlock,
            ...
            if (parentBlock instanceof AILBlockNode) {
                ...
                return ((AILRootNode) parentBlock).getName();
            }
        }
        ...
        @ExportMessage
        @SuppressWarnings({ "static-method", "unused" })
        boolean hasScopeParent(@Shared("block") @Cached(value = "this.block", adopt = false) AILBlockNode cachedBlock,
                @Shared("parentBlock") @Cached(value = "this.block.findBlock()", adopt = false, allowUncached = true) Node parentBlock) {
            // Cache the parent block for the fast-path access
            return parentBlock instanceof AILBlockNode;
        }
        ...
        @ExportMessage
        @SuppressWarnings("unused")
        Object getScopeParent(@Shared("block") @Cached(value = "this.block", adopt = false) AILBlockNode cachedBlock,
                @Shared("parentBlock") @Cached(value = "this.block.findBlock()", adopt = false, allowUncached = true) Node parentBlock)
            ...
            return new VariablesObject(frame, cachedBlock, true, (AILBlockNode) parentBlock);
        }
        ...
        SourceSection getSourceLocation() {
            ...
            if (parentBlock instanceof RootNode) {
                ...
                assert parentBlock instanceof AILRootNode : String.format("In SLLanguage we expect SLRootNode, not %s",
                        parentBlock.getClass());
                return ((AILRootNode) parentBlock).getSourceSection();
            } else {
        ...
        static final class WriteMember {
            ...
            static void doCached(VariablesObject receiver, String member, Object value,
                    ...
                    @Cached(value = "receiver.findWriteNode(member)", adopt = false) AILWriteLocalVariableNode writeNode)
                    throws UnknownIdentifierException, UnsupportedMessageException {
            ...
            static void doGeneric(VariablesObject receiver, String member, Object value)
                    throws UnknownIdentifierException, UnsupportedMessageException {
                AILWriteLocalVariableNode writeNode = receiver.findWriteNode(member);
                doWrite(receiver, member, writeNode, value);
            }

            private static void doWrite(VariablesObject receiver, String member, AILWriteLocalVariableNode writeNode,
        ...
        @SuppressWarnings("static-method")
        Object getMembers(@SuppressWarnings("unused") boolean includeInternal,
                @Cached(value = "this.block.getDeclaredLocalVariables()", adopt = false, dimensions = 1, allowUncached = true) AILWriteLocalVariableNode[] writeNodes,
        ...
        FrameSlot findSlot(String member) {
            AILWriteLocalVariableNode writeNode = findWriteNode(member);
            if (writeNode != null) {
         ...
         */
        AILWriteLocalVariableNode findWriteNode(String member) {
            AILWriteLocalVariableNode[] writeNodes = block.getDeclaredLocalVariables();
    ...
    static final class KeysArray implements TruffleObject {
        ...
        private final AILWriteLocalVariableNode[] writeNodes;
         ...
         */
        KeysArray(AILWriteLocalVariableNode[] writeNodes, int variableIndex, int parentBlockIndex) {
    ...
    /**
     * Representation of a variable based on a {@link AILWriteLocalVariableNode write node} that
    ...
    static final class Key implements TruffleObject {
        ...
        private final AILWriteLocalVariableNode writeNode;

        Key(AILWriteLocalVariableNode writeNode) {
        ...
        SourceSection getSourceLocation() throws UnsupportedMessageException {
            ...
            AILExpressionNode nameNode = writeNode.getNameNode();
